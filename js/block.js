...
function Block(protoblock, blocks, overrideName) {
	...
	// Let the mouse events on the text be handled by the container.
        ...
        var __processHighlightCollapseBitmap = function (bitmap, that) {
            ...
            if (that.collapseText === null) {
                ...
                case 'temperament':
                    ...
                    break;
                default:
                    that.collapseText = new createjs.Text('unknown', fontSize + 'px Sans', platformColor.blockText);
                }
	...
	// We don't need hit area unless we want to restrict the click to some portion of the visible pixels.
	...
	console.log('load event handlers for ' + this.name);
        var that = this;
        var thisBlock = this.blocks.blockList.indexOf(this);
	    ...
	    console.log('mouseover ' + that.name);
        ...
        console.log('window has mouse: ' + window.hasMouse);

        this.container.on('click', function (event) {
	    console.log('click ' + that.name);
		...
		console.log('locked');
            ...
            console.log(window.hasMouse + ' ' + getInput + ' ' + moved + ' ' + dx);
		...
		console.log('why am I here?');
		    ...
		    console.log(that.blocks.getLongPressStatus());
		    console.log(that.blocks.stageClick);
	    ...
	    console.log('mousedown ' + that.name);
            docById('contextWheelDiv').style.display = 'none';
		...
		console.log('pressmove ' + that.name);
                // FIXME: More voodoo
		...
		console.log('pressmove2 ' + that.name);
                // FIXME: More voodoo
                event.nativeEvent.preventDefault();
	    ...
	    console.log('mouseout2 ' + that.name);
            if (!that.blocks.getLongPressStatus()) {
                that._mouseoutCallback(event, moved, haveClick, false);
            } else {
                clearTimeout(that.blocks.longPressTimeout);
	    ...
	    console.log('pressup2 ' + that.name);
            if (!that.blocks.getLongPressStatus()) {
                that._mouseoutCallback(event, moved, haveClick, false);
            } else {
                clearTimeout(that.blocks.longPressTimeout);
    ...
    this.piemenuBlockContext = function (thisBlock) {
	...
	var that = this;
        ...
        wheel.navItems[3].navigateFunction = function () {
            that.blocks.activeBlock = thisBlock;
            that.blocks.extract();
            that.blocks.sendStackToTrash(that);
            docById('contextWheelDiv').style.display = 'none';
        };
        ...
        if (this.name === 'action') {
            wheel.navItems[5].navigateFunction = function () {
                that.blocks.activeBlock = activeBlock;
